...
class GnipPowerTrack(object):
	...
	def addRule(self, tag, value):
		...
		self._sendPowertrackRequest(url=url, method="POST", data=rules_json)
		return True
	...
	def removeRule(self, value):
		...
		rules_json = json.dumps(rules)
		url = ("%(url)s?%(query)s") % {"url": self._buildRulesURL(),
	...
	def getRules(self):
		...
		url = self._buildRulesURL()
		response = self._sendPowertrackRequest(url=url, method="GET")
		return json.loads(response).get("rules")

	def _sendPowertrackRequest(self, url, method, data=None):
	...
	def _buildRulesURL(self):
		...
		url = ("https://gnip-api.twitter.com/rules/powertrack"
			"/accounts/%(account_name)s"
			"/publishers/%(data_source)s"
			"/%(stream_label)s"
			".json") % {"account_name": self.GNIP_ACCOUNT_NAME,
						"data_source": self.GNIP_DATA_SOURCE,
						"stream_label": self.GNIP_STREAM_LABEL}
